...
def display_firewall_results(rate_df, yara_df, blocked_df, allowed_df, dns_df):
    """Helper function to display firewall analysis results"""
    ...
    if not allowed_df.empty and not blocked_df.empty:
...
def firewall_tab():
    ...
    if 'firewall_data' not in st.session_state:
        st.session_state.firewall_data = None
    if 'firewall_dfs' not in st.session_state:
        st.session_state.firewall_dfs = {}
    if 'firewall_just_loaded' not in st.session_state:
        st.session_state.firewall_just_loaded = False

    button_clicked = st.button("Activate Firewall & Analyze Traffic")
    
    if button_clicked:
        st.session_state.firewall_just_loaded = True
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    try:
                        ...
                        st.session_state.firewall_data = True

                        # Display the results
                        display_firewall_results(rate_df, yara_df, blocked_df, allowed_df, dns_df)
                ...
                st.session_state.firewall_data = None
    # Display data from session state if available (when switching tabs or page reload)
    # Only display if button was NOT just clicked (to avoid duplicates)
    if not st.session_state.firewall_just_loaded and st.session_state.firewall_data and st.session_state.firewall_dfs:
        dfs = st.session_state.firewall_dfs
        display_firewall_results(
            dfs.get('rate_df', pd.DataFrame()),
            dfs.get('yara_df', pd.DataFrame()),
            dfs.get('blocked_df', pd.DataFrame()),
            dfs.get('allowed_df', pd.DataFrame()),
            dfs.get('dns_df', pd.DataFrame())
        )
    
    # Reset the flag after displaying
    if st.session_state.firewall_just_loaded:
        st.session_state.firewall_just_loaded = False